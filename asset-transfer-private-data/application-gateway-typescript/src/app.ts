...
async function main(): Promise<void> {
    ...
    try {
        ...
        let requester_quantity = await readassetquery(contractOrg1, assetID1, 'org2');
        let current_asset_quantity = await readAssetPrivateDetails(contractOrg1, 'org1' + assetID1, org1PrivateCollectionName);
        // Input JSON
        let input = {
            a: current_asset_quantity.toString(),
        ...
        totalgentime += await generateproof_send(contractOrg1, assetID1, input);
        ...
        // Read transfer agreement.
        requester_quantity = await readassetquery(contractOrg1, assetID1, 'org3');
        current_asset_quantity = await readAssetPrivateDetails(contractOrg1, 'org1' + assetID1, org1PrivateCollectionName);
        // Input JSON
        input = {
            a: current_asset_quantity.toString(),
            b: requester_quantity.toString()
        ...
        // Read transfer agreement.
        requester_quantity = await readassetquery(contractOrg1, assetID1, 'org4');
        current_asset_quantity = await readAssetPrivateDetails(contractOrg1, 'org1' + assetID1, org1PrivateCollectionName);
        // Input JSON
        input = {
            a: current_asset_quantity.toString(),
        ...
        // Read transfer agreement.
        requester_quantity = await readassetquery(contractOrg1, assetID1, 'org5');
        current_asset_quantity = await readAssetPrivateDetails(contractOrg1, 'org1' + assetID1, org1PrivateCollectionName);
        // Input JSON
        input = {
            a: current_asset_quantity.toString(),
        ...
        // Read transfer agreement.
        requester_quantity = await readassetquery(contractOrg1, assetID1, 'org6');
        current_asset_quantity = await readAssetPrivateDetails(contractOrg1, 'org1' + assetID1, org1PrivateCollectionName);
        // Input JSON
        input = {
            a: current_asset_quantity.toString(),
        ...
        fs.appendFileSync('zkp_files/generation_time.txt', `5:${totalgentime}\n`);
        await purgeAsset(contractOrg1, assetID1);
        await purgeAsset(contractOrg2, assetID1);
        await purgeAsset(contractOrg3, assetID1);
        await purgeAsset(contractOrg4, assetID1);
        await purgeAsset(contractOrg5, assetID1);
        await purgeAsset(contractOrg6, assetID1);
    } finally {
        gatewayOrg1.close();
        clientOrg1.close();
...
async function readassetquery(contract: Contract, assetID: string, sender_org: string): Promise<number> {
    // Seller retrieves the query sent by a buyer

    console.log('\n--> Submit Transaction: ReadAssetQuery, assetID:', assetID);

    const resultBytes = await contract.evaluateTransaction('ReadAssetQuery', assetID, sender_org);
    const resultString = utf8Decoder.decode(resultBytes);
    if (!resultString) {
...
async function generateproof_send(contract: Contract, assetID: string, input: { a: string; b: string }): Promise<number> {
