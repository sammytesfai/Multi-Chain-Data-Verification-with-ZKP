...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    . organizations/cfssl/registerEnroll.sh # Look into script for changes to totalorgs
    for ((i=1; i<=totalorgs; i++))
    do

      infoln "Creating Org$i Identities"
      #function_name cert-type   CN   org
      peer_cert peer peer0.org$i.example.com org$i
      peer_cert admin Admin@org$i.example.com org$i
    done
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    ${CONTAINER_CLI_COMPOSE} -f compose/$COMPOSE_FILE_CA -f compose/$CONTAINER_CLI/${CONTAINER_CLI}-$COMPOSE_FILE_CA up -d --remove-orphans 2>&1
...
function createChannel() {
  ...
  scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true $ORG1 $ORG2
}
...
function deployCC() {
  org1=$1
  org2=$2
  scripts/deployCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $org1 $org2
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    for ((i=1; i<=totalorgs; i++))
    do
      ${CONTAINER_CLI} volume rm compose_peer0.org$i.example.com
    done
    ...
    for ((i=1; i<=totalorgs; i++))
    do
      ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c "cd /data && rm -rf organizations/fabric-ca/org$i/msp organizations/fabric-ca/org$i/tls-cert.pem organizations/fabric-ca/org$i/ca-cert.pem organizations/fabric-ca/org$i/IssuerPublicKey organizations/fabric-ca/org$i/IssuerRevocationPublicKey organizations/fabric-ca/org$i/fabric-ca-server.db"
    done
